...
public class FileChooserAdapter extends BaseAdapter {
	...
	private static ArrayList<String> BIN_SUFFIX = new ArrayList<String>();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (fileInfo.isDirectory()) { // 閺傚洣娆㈡径锟�			holder.imgFileIcon.setImageResource(R.drawable.ic_folder);
			holder.tvFileName.setTextColor(Color.GRAY);
		} else if (fileInfo.isBinFile()) {
			holder.imgFileIcon.setImageResource(R.drawable.ic_bin);
	...
	public static class FileInfo {
		...
		public boolean isBinFile() {
			if (fileName.lastIndexOf(".") < 0) // Don't have the suffix
				return false;
			String fileSuffix = fileName.substring(fileName.lastIndexOf("."));
			if (!isDirectory() && BIN_SUFFIX.contains(fileSuffix))
				return true;
			else
				return false;
		}
